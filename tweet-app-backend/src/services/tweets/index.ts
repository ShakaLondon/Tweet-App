...
tweetRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const userID = req.params._id ? req.params._id : (user as IUser)._id
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (Array.isArray(mentionArray) && mentionArray.length > 0) {
        ...
        mentionArray.forEach( async (word: string) => {
      ...
      await hashtagArray.forEach( async (word: string) => {
