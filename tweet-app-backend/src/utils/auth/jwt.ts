...
import { IJWTPayload, IRole, ITokenRequest, IUser } from "../../types/schema-types.js";
  ...
  try {
    ...
    if (!req.headers.authorization) {
      ...
      try {
        ...
        (req as ITokenRequest).token = token;
        req.params._id = (decoded as JwtPayload).id;
...
export async function JwtUserMiddleware(req: Request, res: Response, next: NextFunction) {
  try {
...
export const adminOnly = (req: Request, res: Response, next: NextFunction) => {
  ...
  UserModel.findById(req.params.loginID, (err: Error, user: IUser) => {
    if (err) {
...
export const developerOnly = (req: Request, res: Response, next: NextFunction) => {
...
function reject(err: jwt.VerifyErrors) {
