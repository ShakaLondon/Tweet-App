...
import { IQuery, IKafkaResponse, SimpleTweetConsumer } from "../../../types/kafka-types.js";
import { randomUUID } from "crypto";

/**
 * Consumes and decodes Orchestrate messages
 */
export default class TweetConsumer implements SimpleTweetConsumer{
  private consumer: Consumer
  private kafka: Kafka
  private topics: string
  tweetsOutArray: IKafkaResponse[] = []
  ...
  public async consumeTweetPartition( partition: String ): Promise<IKafkaResponse[] | undefined> {
  ...
  public async consumeSingleTweetType( query: IQuery ): Promise<IKafkaResponse[] | undefined> {
    // Not absolutely necessary but enforces tweet to call connect() before calling consume()
    try {
      ...
      this.consumer.run({
        eachMessage: async ({ topic, partition, message }) => {
          console.log(message)
            ...
            this.tweetsOutArray.push(messageOut as IKafkaResponse)
        }
      })
  ...
  private handleMessage( message: KafkaMessage ): IKafkaResponse {
    ...
    return JSON.parse((message.value as Buffer).toString("utf8"))
